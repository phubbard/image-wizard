...
| **About** (`/about`) | Library stats, scan timestamps, **directory multi-select**: check / uncheck which scanned roots are visible across Timeline / Search / Map / Nearby. The selection persists in the DB. A `⌖ filtered` pill appears in the nav of every page when a strict subset is active. The filter is **self-healing**: stored roots that are no longer scan roots — after a prune, or once `collate` re-points files under a new root — are dropped automatically, and if none survive it reverts to *show everything*, so a stale selection can never silently blank the library. |
...
# cluster-faces (→ ocr → suggest-rotations), with an fcntl lock so a slow
# prior run doesn't overlap the next scheduled tick. Silent on success;
# prints one summary line to stderr on failure (so cron mails you only when
# something needs attention). Full output → <cache>/logs/refresh.log.
#
# - find-duplicates (perceptual, --dedupe-index) hides re-imported /
#   re-exported copies so duplicates don't accumulate between runs.
# - suggest-rotations runs only if you've trained the orientation model
#   (train-orientation); otherwise it's skipped, not failed.
# Both are incremental, so they're cheap after the first run.
#
...
image-wizard refresh --no-dedupe --no-suggest-rotations   # skip those phases
```
