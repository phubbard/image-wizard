...
| **Geotag** (`/geotag`) | Workflow for adding locations to photos missing GPS (common on old scans). Walks the untagged photos one at a time; click a spot on the map, and the coordinates are reverse-geocoded offline to city/region/country and stored. The clicked point persists between photos so a same-place batch is repeated "Save & next"; an optional checkbox tags every untagged photo from the same calendar day at once. Reachable from the Map banner or the "+ Add location" link on any photo detail without GPS. |
